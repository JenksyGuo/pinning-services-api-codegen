...
class Pin implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    public function setName($name)
    {
        if (!is_null($name) && (mb_strlen($name) > 255)) {
            throw new \InvalidArgumentException('invalid length for $name when calling Pin., must be smaller than or equal to 255.');
        }
    ...
    public function setOrigins($origins)
    {
        ...
        if (!is_null($origins) && (count($origins) > 20)) {
            throw new \InvalidArgumentException('invalid value for $origins when calling Pin., number of items must be less than or equal to 20.');
        }
        if (!is_null($origins) && (count($origins) < 0)) {
            throw new \InvalidArgumentException('invalid length for $origins when calling Pin., number of items must be greater than or equal to 0.');
        }
